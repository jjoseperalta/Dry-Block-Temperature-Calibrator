...
TOLERANCE_LINE = re.compile(
    r'\[TOLERANCE\]:\s*([0-9.]+)'
)

# NUEVAS: Expresiones regulares para parsear estado del sistema
...
STATE_KEYWORD_LINE = re.compile(r"State:\s*(\w+)")

TEMPERATURE_LINE = re.compile(r"\[TEMP\]\s*Master:\s*(-?\d+\.?\d*)\s*(°[CF])\s*\|\s*Test:\s*(-?\d+\.?\d*)\s*(°[CF])\s*\|\s*Diff:\s*(-?\d+\.?\d*)\s*(°[CF])")
...
class SerialMonitorWithPlot:
    def __init__(self, root):
        ...
        self.root.geometry("970x920")  # Aumentado para la barra de estado
        self.root.minsize(830, 700)
        ...
        # Variables para estado del sistema
        self.last_fan_speed = "0"
        self.last_peltier_mode = None  # 'COOL', 'HEAT', None
        self.last_peltier_power = "0"
        self.last_status_update = time.time()  # Para detectar inactividad
    ...
    def apply_theme(self, theme_name):
        ...
        for name in ['controls_frame', 'left_controls', 'right_controls', 'cmd_frame', 'plot_controls_frame', 'status_bar']:
        ...
        if hasattr(self, 'output_area'):
            ...
            # Tags para colorear palabras clave
    ...
    def _update_status_bar_colors(self):
        """Actualiza los colores de la barra de estado"""
        if hasattr(self, 'fan_status_label'):
            self.fan_status_label.configure(bg=self.bg_color, fg=self.fg_color)
        if hasattr(self, 'peltier_status_label'):
            self.peltier_status_label.configure(bg=self.bg_color, fg=self.fg_color)
        if hasattr(self, 'status_bar_label'):
            self.status_bar_label.configure(bg=self.bg_color, fg=self.fg_color)
        if hasattr(self, 'status_bar_fan'):
            self.status_bar_fan.configure(bg=self.bg_color)
        if hasattr(self, 'status_bar_peltier'):
            self.status_bar_peltier.configure(bg=self.bg_color)
    ...
    def create_status_bar(self):
        ...
        self.status_bar = tk.Frame(self.main_container, bg=self.bg_color, height=32)
        self.status_bar.pack(side=tk.BOTTOM, fill=tk.X)
        self.status_bar.pack_propagate(False)
        
        # Separador visual superior
        separator = tk.Frame(self.status_bar, bg="#555555", height=1)
        separator.pack(side=tk.TOP, fill=tk.X)
        
        # Frame interno para los elementos
        status_inner = tk.Frame(self.status_bar, bg=self.bg_color)
        status_inner.pack(side=tk.LEFT, padx=10, pady=5)
        
        # Etiqueta "Sistema:"
        self.status_bar_label = tk.Label(status_inner, text="⚙️ Sistema:", 
                                         bg=self.bg_color, fg=self.fg_color,
                                         font=("Arial", 9, "bold"))
        self.status_bar_label.pack(side=tk.LEFT, padx=(0, 15))
        
        # Indicador del ventilador
        self.status_bar_fan = tk.Frame(status_inner, bg=self.bg_color)
        self.status_bar_fan.pack(side=tk.LEFT, padx=(0, 15))
        
        self.fan_indicator = tk.Label(self.status_bar_fan, text="🟢", 
                                      bg=self.bg_color, fg="#00aa00",
                                      font=("Arial", 10))
        self.fan_indicator.pack(side=tk.LEFT, padx=(0, 4))
        
        self.fan_status_label = tk.Label(self.status_bar_fan, text="Fan: --%",
                                         bg=self.bg_color, fg=self.fg_color,
                                         font=("Arial", 9))
        self.fan_status_label.pack(side=tk.LEFT)
        
        # Separador
        sep1 = tk.Label(status_inner, text="|", bg=self.bg_color, fg="#555555", font=("Arial", 9))
        sep1.pack(side=tk.LEFT, padx=(0, 15))
        
        # Indicador del Peltier
        self.status_bar_peltier = tk.Frame(status_inner, bg=self.bg_color)
        self.status_bar_peltier.pack(side=tk.LEFT)
        
        self.peltier_indicator = tk.Label(self.status_bar_peltier, text="⚪", 
                                          bg=self.bg_color, fg="#888888",
                                          font=("Arial", 10))
        self.peltier_indicator.pack(side=tk.LEFT, padx=(0, 4))
        
        self.peltier_status_label = tk.Label(self.status_bar_peltier, text="Peltier: --%",
                                             bg=self.bg_color, fg=self.fg_color,
                                             font=("Arial", 9))
        self.peltier_status_label.pack(side=tk.LEFT)
        
    def update_status_bar(self, fan_speed=None, peltier_mode=None, peltier_power=None):
        """Actualiza la barra de estado con los valores proporcionados"""
        ...
        if fan_speed is not None:
            ...
            try:
                ...
                if speed > 0:
                    self.fan_indicator.config(fg="#00aa00", text="🟢")
                else:
                    self.fan_indicator.config(fg="#888888", text="⚪")
            except:
                pass
            self.fan_status_label.config(text=f"Fan: {fan_speed}%")
        
        # Actualizar Peltier
        if peltier_mode is not None:
            self.last_peltier_mode = peltier_mode
            if peltier_mode == "HEAT":
                self.peltier_indicator.config(fg="#ff4444", text="🔴")
                self.peltier_status_label.config(text=f"Calentando: {self.last_peltier_power}%")
            elif peltier_mode == "COOL":
                self.peltier_indicator.config(fg="#44aaff", text="🔵")
                self.peltier_status_label.config(text=f"Enfriando: {self.last_peltier_power}%")
            else:
                self.peltier_indicator.config(fg="#888888", text="⚪")
                self.peltier_status_label.config(text="Peltier: IDLE")
        
        if peltier_power is not None:
            self.last_peltier_power = peltier_power
            if self.last_peltier_mode == "HEAT":
                self.peltier_status_label.config(text=f"Calentando: {peltier_power}%")
            elif self.last_peltier_mode == "COOL":
                self.peltier_status_label.config(text=f"Enfriando: {peltier_power}%")
            else:
                self.peltier_status_label.config(text=f"Peltier: {peltier_power}%")

    def reset_status_bar(self):
        """Pone todos los estados en gris (sin datos)"""
        self.fan_indicator.config(fg="#888888", text="⚪")
        self.fan_status_label.config(text="Fan: --%")
        self.peltier_indicator.config(fg="#888888", text="⚪")
        self.peltier_status_label.config(text="Peltier: --%")
        self.last_fan_speed = "0"
        self.last_peltier_mode = None
        self.last_peltier_power = "0"

    def _update_status_from_line(self, line):
        """Analiza una línea del log para actualizar la barra de estado"""
        # Buscar ventilador
        if m := FAN_PWM_LINE.search(line):
            self.update_status_bar(fan_speed=m.group(1))
        
        # Buscar enfriamiento (Cooling)
        if m := COOLING_PWM_LINE.search(line):
            self.update_status_bar(peltier_mode="COOL", peltier_power=m.group(1))
        
        # Buscar calefacción (Heater)
        if m := HEATER_PWM_LINE.search(line):
            self.update_status_bar(peltier_mode="HEAT", peltier_power=m.group(1))

    def _apply_keyword_coloring(self, text, start_pos=None):
        """Aplica colores a palabras clave en el texto usando búsqueda"""
        ...
        if "RAMPING" in text:
            ...
            while True:
                pos = self.output_area.search("RAMPING", search_start, tk.END)
                if not pos:
                    break
                end_pos = f"{pos}+7c"
                self.output_area.tag_add("ramping", pos, end_pos)
        ...
        if "APPROACHING" in text:
            search_start = start_pos
            while True:
                pos = self.output_area.search("APPROACHING", search_start, tk.END)
                if not pos:
                    break
                end_pos = f"{pos}+11c"
                self.output_area.tag_add("approaching", pos, end_pos)
                search_start = end_pos
        ...
        if "HOLDING" in text:
            search_start = start_pos
            while True:
                pos = self.output_area.search("HOLDING", search_start, tk.END)
                if not pos:
                    break
                end_pos = f"{pos}+7c"
                self.output_area.tag_add("holding", pos, end_pos)
                search_start = end_pos
        
        # Buscar State: con palabras clave (formato alternativo)
        if "State:" in text:
            for keyword, tag in [("RAMPING", "ramping"), ("APPROACHING", "approaching"), ("HOLDING", "holding")]:
                if keyword in text:
                    search_start = start_pos
                    while True:
                        pos = self.output_area.search(keyword, search_start, tk.END)
                        if not pos:
                            break
                        end_pos = f"{pos}+{len(keyword)}c"
                        self.output_area.tag_add(tag, pos, end_pos)
                        search_start = end_pos
    ...
    def start_plot_updates(self):
        def update():
            ...
            if self.is_connected:
                ...
                # Resetear barra de estado si pasaron más de 5 segundos sin datos de ventilador/peltier
                time_since_status = time.time() - self.last_status_update
    ...
    def connect(self):
        ...
        try:
            ...
            self.last_status_update = time.time()  # Resetear estado
    ...
    def disconnect(self):
        ...
        # Resetear estado de la barra inferior
        self.reset_status_bar()
            ...
            try:
                if self.serial_connection.in_waiting:
                    ...
                    if data:
                        ...
                        # Procesar la línea para actualizar la barra de estado
                        self._update_status_from_line(decoded_data)
    ...
    def append_output(self, text, is_error=False):
        ...
        # Guardar la posición ANTES de insertar
        ...
        # Aplicar colores a palabras clave (solo si no es error)
        if not is_error:
            # La búsqueda se hará desde donde empezamos a insertar
            self._apply_keyword_coloring(text, start_pos)
