  ...
  while (true) {
    ...
    if (currentState == ControlState::RUNNING) {
      ...
      case ThermalState::HOLDING:
        ...
        // if (dTdt < -0.01f && currentMasterTemp < (setpoint + 0.10f) && error < 0.3f) {
        // if (dTdt < -0.01f && currentMasterTemp > setpoint && error < 0.3f) {
        //   if (millis() - ultimoPulsoFreno > 3000) {
        //     frenoExtra = fabs(dTdt) * 8.0f + 0.5f;
        //     ultimoPulsoFreno = millis();
        //     logf("!!! FRENO INTELIGENTE !!! Potencia: +%.1f%%\n", frenoExtra);
        //   }
        // }
        // Solo aplicar freno si realmente estamos cayendo rápido cerca del setpoint
        
        if (dTdt < -0.02f && currentMasterTemp > (setpoint - 0.2f)) {
            frenoExtra = fabs(dTdt) * 5.0f; // Bajamos el multiplicador de 8 a 5
        }
        ...
        // 1. Caso: Queremos ENFRIAR el bloque (Setpoint por debajo de la temperatura actual)
        ...
        // 2. Caso: Queremos CALENTAR o MANTENER calor (Setpoint arriba o cerca del actual)
        else {
